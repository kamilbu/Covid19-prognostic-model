...
CREATE or replace TABLE norway_covid as
--select * from 
...
case when date_reported < '2021-03-30' then 0
...
case when date_reported < '2021-01-06' then 0 
...
(select * from covid_global where country='Norway') cases
left join 
(select * from vaccination1 where location='Norway') vacc
on cases.date_reported=vacc.date)
where date_reported >= '2020-01-31'
                               )
