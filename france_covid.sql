...
CREATE or replace TABLE france_covid as
--select * from 
...
case when date_reported < '2020-12-27' then 0 
else total_vaccinations end as 
total_vaccinations,
case when date_reported < '2020-12-27' then 0 
else people_vaccinated end as people_vaccinated, 
case when date_reported < '2021-01-02' then 0 
else people_fully_vaccinated end as people_fully_vaccinated, 
case when date_reported < '2021-01-26' then 0
else total_boosters end as total_boosters, 
case when date_reported < '2020-12-03' then 0 
when date_reported='2020-12-27' then total_vaccinations
...
case when date_reported < '2020-12-03' then 0 
when date_reported='2020-12-03' then people_vaccinated 
...
(select * from covid_global where country='France') cases
left join 
(select * from vaccination1 where location='France') vacc
on cases.date_reported=vacc.date)
where date_reported >= '2020-01-24'
                               )
