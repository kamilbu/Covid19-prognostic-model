...
#wczytanie csvek i ustawienie że kolumna z datą to serio data
...
#tu się robi index z daty ale nie wiem czy on jest potrzebny
...
#dodawanie nowych kolumn - miesiac, dzien tygodnia itd
...
#ta funkcja powinna generowac wektory wartosci z poprzednich dni
def generate_t_series(input, value_num):
    input=input.to_numpy()
    new_cases = input[:,2]
    generator = TimeseriesGenerator(new_cases,new_cases, 
                               length=value_num,
                               batch_size=len(new_cases))
    global_index = value_num
    i, t = generator[0]
    for b_row in range(len(t)):
        assert(abs(t[b_row] - new_cases[global_index]) <= 0.001)          
        global_index += 1
    return i, t
              


#print (poland_covid.head())

X, Y = generate_t_series(poland_covid, 4)
Y = Y.reshape((Y.shape[0],1))

#print(Y)
#print(X)
#close = poland_covid.to_numpy()
#print(close[:,3])
#print(Y)
#print(X)
#X=np.flip(X,1)
vector_new_cases=np.concatenate((X, Y), axis=1)
#print(vector_new_cases)
...
parameters_X=parameters_X.iloc[4:,:].to_numpy().astype(int)

#concat, dzien liczony na koncu
vector_new_cases=np.concatenate((parameters_X, vector_new_cases), axis=1)
#wyswietlanko
np.set_printoptions(threshold=np.inf)

#print(vector_new_cases)
X=vector_new_cases[:,:-1]

scaler_filename = "scaler"
scaler = MinMaxScaler(feature_range=(0, 1))
scaled = scaler.fit_transform(vector_new_cases)
X_scaled=scaled
...
#training
history = model.fit(X_train, y_train, epochs=2000 , batch_size=5, validation_split=0.2, verbose=2)
 """
...
plt.title('Prognozy w porównaniu z ilością nowych przypadków', fontdict={'fontsize': 18, 'fontweight': 'medium'})
plt.xlabel('próbki 1-100', fontsize=15)
plt.ylabel('ilość przypadków', fontsize=15)
plt.subplot(2,1,2)
